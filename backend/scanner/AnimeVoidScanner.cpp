...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN:
    {
        ...
        {
            MessageBox(NULL, L"Start butonu basıldı!", L"Debug", MB_OK | MB_ICONINFORMATION);
            OnStartButtonClick();
        }
        return 0;
    }
...
void DrawAnimeCharacter(HDC hdc, RECT rect)
{
    ...
    // Body
    Rectangle(hdc, 320, 200, 380, 300);
    
    // Arms
    Rectangle(hdc, 280, 220, 320, 240);
    Rectangle(hdc, 380, 220, 420, 240);
    
    // Legs
    Rectangle(hdc, 330, 300, 350, 380);
    Rectangle(hdc, 350, 300, 370, 380);
    
    // Eyes
    Ellipse(hdc, 320, 130, 335, 145);
    Ellipse(hdc, 365, 130, 380, 145);
    
    // Katana
    Rectangle(hdc, 450, 150, 455, 250);
    Rectangle(hdc, 440, 140, 465, 160);
...
void DrawRainEffect(HDC hdc, RECT rect)
{
    ...
    HPEN rainPen = CreatePen(PS_SOLID, 1, RGB(50, 50, 50));
    HPEN oldPen = (HPEN)SelectObject(hdc, rainPen);

    // Draw rain drops
    for (int i = 0; i < 100; i++)
    {
        int x = (i * 7 + animationFrame) % rect.right;
        int y = (animationFrame * 3 + i * 5) % rect.bottom;
...
void OnStartButtonClick()
{
    ...
    CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)PerformScan, NULL, 0, NULL);
}
...
string TakeScreenshot()
{
    try
    {
        ...
        // Save bitmap to file
        string filename = "void_screenshot_" + to_string(time(NULL)) + ".png";
        string filepath = string(getenv("TEMP")) + "\\" + filename;

        // Simple bitmap save (you might want to use GDI+ for PNG)
        // For now, save as BMP
        string bmpPath = filepath.substr(0, filepath.length() - 4) + ".bmp";
        ...
        HANDLE hFile = CreateFileA(bmpPath.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void SendResults(const string& screenshotPath)
{
    try
    {
        ...
        if (hInternet)
        {
            ...
            {
                // Prepare JSON data
                string results = "VOID SCANNER - C++ SCAN COMPLETED\n" +
